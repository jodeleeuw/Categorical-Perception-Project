...
sd_classic_cp_rel0 <- ddply(filterdata[filterdata$trial_type=="same-different" & filterdata$ydist==0 & filterdata$distance > 0,],
                                .(mturk_id, train_type, stim_type),
                                function(subset)with(subset, c(mean_score = mean(correct))))

sd_classic_cp_irrel0 <- ddply(filterdata[filterdata$trial_type=="same-different" & filterdata$xdist==0 & filterdata$distance > 0,],
                                  .(mturk_id, train_type, stim_type),
                                  function(subset)with(subset, c(mean_score = mean(correct))))
